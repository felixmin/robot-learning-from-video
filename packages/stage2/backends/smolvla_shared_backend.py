...
class SmolVLASharedBackend(torch.nn.Module):
    ...
    def _require_target_actions(self, batch: Stage2Batch) -> torch.Tensor:
        ...
        if actions.ndim not in (2, 3):
            raise ValueError(
                f"Expected target_actions [B,A] or [B,T,A], got {tuple(actions.shape)}"
            )
        if self.cfg.action_dim is None:
            raise ValueError("Action mode requires action_dim in backend config")
        actions = to_action_chunk(
            actions=actions, chunk_size=int(self.cfg.action_chunk_size)
        )
    ...
    def loss_from_batch(self, batch: Stage2Batch, *, mode: BackendMode) -> LossOutput:
        if mode is BackendMode.LATENT_FLOW:
            ...
            latent_loss = latent_flow_loss(
                core=self.core, batch=batch, target_vectors=target_vec
            )
        ...
        if mode is BackendMode.MULTITASK:
            ...
            total = (
                float(self.cfg.latent_loss_weight) * latent_loss
                + float(self.cfg.action_loss_weight) * action_loss
            )
            return LossOutput(
                loss=total,
                metrics={
    ...
    def latent_from_batch(
        ...
        if mode is BackendMode.LATENT_FLOW:
            ...
            return LatentOutput(
                logits=None, tokens=None, vector=vec, actions=None, meta=None
            )

        if mode is BackendMode.ACTIONS:
            actions = self.core.predict_actions(batch=batch)
            return LatentOutput(
                logits=None, tokens=None, vector=None, actions=actions, meta=None
            )
        ...
        if mode is BackendMode.MULTITASK:
            ...
            actions = self.core.predict_actions(batch=batch)
            return LatentOutput(
                logits=None, tokens=None, vector=vec, actions=actions, meta=None
            )
