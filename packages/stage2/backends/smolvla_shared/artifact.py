...
class SmolVLASharedArtifactManifest:
    ...
    def validate(self) -> None:
        ...
        if len(self.image_size) != 2:
            raise ValueError(
                f"manifest.image_size must have length 2, got {self.image_size!r}"
            )
        if int(self.image_size[0]) <= 0 or int(self.image_size[1]) <= 0:
            raise ValueError(
                f"manifest.image_size values must be > 0, got {self.image_size!r}"
            )
        ...
        if int(self.action_chunk_size) <= 0:
            raise ValueError(
                f"manifest.action_chunk_size must be > 0, got {self.action_chunk_size}"
            )
        if int(self.code_seq_len) <= 0:
            raise ValueError(
                f"manifest.code_seq_len must be > 0, got {self.code_seq_len}"
            )
        if int(self.latent_vector_dim) <= 0:
            raise ValueError(
                f"manifest.latent_vector_dim must be > 0, got {self.latent_vector_dim}"
            )
        ...
        if int(self.max_state_dim) <= 0:
            raise ValueError(
                f"manifest.max_state_dim must be > 0, got {self.max_state_dim}"
            )
        if int(self.flow_hidden_dim) <= 0:
            raise ValueError(
                f"manifest.flow_hidden_dim must be > 0, got {self.flow_hidden_dim}"
            )
        ...
        if float(self.time_beta_alpha) <= 0.0:
            raise ValueError(
                f"manifest.time_beta_alpha must be > 0, got {self.time_beta_alpha}"
            )
        if float(self.time_beta_beta) <= 0.0:
            raise ValueError(
                f"manifest.time_beta_beta must be > 0, got {self.time_beta_beta}"
            )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "system_prompt": (
                None if self.system_prompt is None else str(self.system_prompt)
            ),
            "max_state_dim": int(self.max_state_dim),
            "camera_keys": (
                None if self.camera_keys is None else [str(k) for k in self.camera_keys]
            ),
            ...
            "source_training_mode": (
                None
                if self.source_training_mode is None
                else str(self.source_training_mode)
            ),
            "source_run_dir": (
                None if self.source_run_dir is None else str(self.source_run_dir)
            ),
            "source_global_step": (
                None
                if self.source_global_step is None
                else int(self.source_global_step)
            ),
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> SmolVLASharedArtifactManifest:
        manifest = cls(
            ...
            system_prompt=(
                None if raw["system_prompt"] is None else str(raw["system_prompt"])
            ),
            ...
            source_training_mode=(
                None
                if raw["source_training_mode"] is None
                else str(raw["source_training_mode"])
            ),
            source_run_dir=(
                None if raw["source_run_dir"] is None else str(raw["source_run_dir"])
            ),
            source_global_step=(
                None
                if raw["source_global_step"] is None
                else int(raw["source_global_step"])
            ),
...
def _validate_tensor_state_dict(
    name: str, state_dict: Mapping[str, Any]
    ...
    for key, value in state_dict.items():
        ...
        if not torch.is_tensor(value):
            raise TypeError(
                f"{name}[{key!r}] must be a torch.Tensor, got {type(value)}"
            )
        clean[key] = value
    return clean


def _filter_export_state_dict(
...
def load_smolvla_shared_artifact(
    *,
    ...
    core_state_dict = _validate_tensor_state_dict(
        "core_state_dict", payload["core_state_dict"]
    )
